...
public class LinkedList<E>
{
	...
	public boolean add(E obj)
	{
		if(size == 0) {
			...
			last.setNext(new DoubleNode(obj));
	...
	private class LinkedListIterator implements Iterator<E>
	{
		...
		/**
		 * Inserts a new object before the most recently returned object.
		 * @param ins	the object to insert
		 */
		public void insert(E ins) {
			DoubleNode newNode = new DoubleNode(ins);
			if(lastNode == null) {
				newNode.setNext(first);
				if(first != null) first.setPrevious(newNode);
				first = newNode;
			} else {
				newNode.setPrevious(lastNode.getPrevious());
				lastNode.setPrevious(newNode);
				newNode.setNext(lastNode);
				if(newNode.getPrevious() != null) newNode.getPrevious().setNext(newNode);
				else first = newNode;
			}
			size++;
		}
